...
/* Force dark background even when system/browser uses light mode */
[data-testid="stApp"]{background-color:#0b0f1a!important;}
[data-testid="block-container"]{background-color:#0b0f1a!important;}
.stMarkdown p,.stMarkdown span{color:#e2e8f0;}
...
[data-testid="stSelectbox"] label,[data-testid="stNumberInput"] label{color:#94a3b8!important;}
div[data-baseweb="tab-list"]{background:#111827!important;border-bottom:1px solid #1e3a5f!important;}
div[data-baseweb="tab-highlight"]{background:#0ea5e9!important;}
.stDataFrame{background:#111827!important;}
thead tr th{background:#1a2236!important;color:#e2e8f0!important;font-family:'IBM Plex Mono',monospace!important;}
tbody tr td{color:#e2e8f0!important;font-family:'IBM Plex Mono',monospace!important;}
tbody tr:nth-child(even) td{background:#111827!important;}
tbody tr:nth-child(odd) td{background:#0d1520!important;}
p{color:var(--text);}
</style>
...
def ukc_badge(status: str, ukc: float, required: float,
    ...
    """Render the UKC Meter gauge using st.components for reliable SVG rendering."""
    import math, streamlit.components.v1 as components

    # ── Colours ──────────────────────────────────────────────────────────────
    clr  = {"ADEQUATE": "#10b981", "MARGINAL": "#f59e0b"}.get(status, "#ef4444")
    icon = {"ADEQUATE": "✓",       "MARGINAL": "⚠"      }.get(status, "✕")
    cls  = {"ADEQUATE": "badge-ok","MARGINAL": "badge-warn"}.get(status,"badge-risk")
    badge_bg   = {"ADEQUATE":"rgba(16,185,129,0.18)","MARGINAL":"rgba(245,158,11,0.18)"}.get(status,"rgba(239,68,68,0.18)")
    border_clr = {"ADEQUATE":"rgba(16,185,129,0.35)","MARGINAL":"rgba(245,158,11,0.35)"}.get(status,"rgba(239,68,68,0.45)")

    # ── Needle angle ──────────────────────────────────────────────────────────
    # Gauge: 0° = full safe clearance, 220° arc total
    # Needle moves from LEFT (safe) toward RIGHT (aground) as draft approaches depth
    if depth_m and depth_m > 0 and draft_m is not None:
        frac = min(1.0, max(0.0, draft_m / depth_m))
    else:
        # Fall back: use UKC vs required as proxy
        frac = max(0.0, 1.0 - min(1.0, (ukc / max(required, 0.01)) * 0.5)) if required > 0 else 0.3
    ...
    R_inner  = 66
    R_needle = 86
    START    = 200   # leftmost angle (degrees CCW from east)
    SWEEP    = 220
    ...
    # Zone boundary angles (CCW)
    z_red   = START - SWEEP          # rightmost = most dangerous
    z_amber = START - SWEEP * 0.30
    z_green = START - SWEEP * 0.60

    # Needle angle
    needle_ang = START - frac * SWEEP

    def arc(r_o, r_i, a1, a2, fill):
        """Donut arc path from angle a1 to a2 (both CCW from east)."""
        large = 1 if abs(a2 - a1) > 180 else 0
        o1x, o1y = px(r_o,a1), py(r_o,a1)
        o2x, o2y = px(r_o,a2), py(r_o,a2)
        i1x, i1y = px(r_i,a1), py(r_i,a1)
        i2x, i2y = px(r_i,a2), py(r_i,a2)
        return (f'<path d="M {o1x:.1f} {o1y:.1f} '
                f'A {r_o} {r_o} 0 {large} 0 {o2x:.1f} {o2y:.1f} '
                f'L {i2x:.1f} {i2y:.1f} '
                f'A {r_i} {r_i} 0 {large} 1 {i1x:.1f} {i1y:.1f} Z" '
                f'fill="{fill}"/>')
    ...
    for i in range(11):
        ...
        r1 = R_outer - 2
        r2 = R_outer - (8 if i%5==0 else 4)
        ticks += (f'<line x1="{px(r1,ang):.1f}" y1="{py(r1,ang):.1f}" '
                  f'x2="{px(r2,ang):.1f}" y2="{py(r2,ang):.1f}" '
                  f'stroke="#2d4a6a" stroke-width="1.5" stroke-linecap="round"/>')

    # Needle polygon (triangle pointing outward)
    n_tip_x, n_tip_y   = px(R_needle, needle_ang), py(R_needle, needle_ang)
    n_b1_x, n_b1_y     = px(7, needle_ang+90), py(7, needle_ang+90)
    n_b2_x, n_b2_y     = px(7, needle_ang-90), py(7, needle_ang-90)

    svg = f"""<svg viewBox="0 0 {W} {H}" xmlns="http://www.w3.org/2000/svg"
      style="width:100%;max-width:{W}px;display:block;margin:0 auto">
  <defs>
    <radialGradient id="g1" cx="50%" cy="70%" r="60%">
      <stop offset="0%" stop-color="#162236"/>
  ...
  <!-- Background disc -->
  <circle cx="{cx}" cy="{cy}" r="{R_outer+10}" fill="url(#g1)" stroke="#1a3a5c" stroke-width="1"/>

  <!-- Track (full arc background) -->
  {arc(R_outer, R_inner, z_red, START, "#0f1e30")}

  <!-- RED zone (right/danger) -->
  {arc(R_outer-1, R_inner+1, z_red, z_amber, "rgba(239,68,68,0.25)")}
  <!-- AMBER zone (middle) -->
  {arc(R_outer-1, R_inner+1, z_amber, z_green, "rgba(245,158,11,0.22)")}
  <!-- GREEN zone (left/safe) -->
  {arc(R_outer-1, R_inner+1, z_green, START, "rgba(16,185,129,0.22)")}

  <!-- Zone dividers -->
  <line x1="{px(R_inner,z_amber):.1f}" y1="{py(R_inner,z_amber):.1f}" x2="{px(R_outer,z_amber):.1f}" y2="{py(R_outer,z_amber):.1f}" stroke="#1a3a5c" stroke-width="1.5"/>
  <line x1="{px(R_inner,z_green):.1f}" y1="{py(R_inner,z_green):.1f}" x2="{px(R_outer,z_green):.1f}" y2="{py(R_outer,z_green):.1f}" stroke="#1a3a5c" stroke-width="1.5"/>

  <!-- Tick marks -->
  {ticks}

  <!-- SAFE / RISK labels -->
  <text x="{px(R_outer+14, START-8):.1f}" y="{py(R_outer+14, START-8):.1f}" font-family="IBM Plex Mono,monospace" font-size="7.5" fill="#10b981" font-weight="700" text-anchor="middle">SAFE</text>
  <text x="{px(R_outer+14, z_red+8):.1f}" y="{py(R_outer+14, z_red+8):.1f}" font-family="IBM Plex Mono,monospace" font-size="7.5" fill="#ef4444" font-weight="700" text-anchor="middle">RISK</text>

  <!-- Needle shadow -->
  <polygon points="{n_b1_x:.1f},{n_b1_y:.1f} {n_b2_x:.1f},{n_b2_y:.1f} {n_tip_x:.1f},{n_tip_y:.1f}"
           fill="rgba(0,0,0,0.35)" transform="translate(2,2)"/>
  <!-- Needle -->
  <polygon points="{n_b1_x:.1f},{n_b1_y:.1f} {n_b2_x:.1f},{n_b2_y:.1f} {n_tip_x:.1f},{n_tip_y:.1f}"
           fill="{clr}" filter="url(#glow)"/>

  <!-- Hub -->
  <circle cx="{cx}" cy="{cy}" r="10" fill="#0b1220" stroke="{clr}" stroke-width="2.5"/>
  <circle cx="{cx}" cy="{cy}" r="4.5" fill="{clr}"/>

  <!-- UKC value (centre of dial) -->
  <text x="{cx}" y="{cy-26}" font-family="IBM Plex Mono,monospace" font-size="21"
        font-weight="700" fill="{clr}" text-anchor="middle" filter="url(#glow)">{ukc:.2f}</text>
  <text x="{cx}" y="{cy-11}" font-family="IBM Plex Sans,sans-serif" font-size="8"
        fill="#94a3b8" text-anchor="middle">metres UKC</text>

  <!-- Status chip -->
  <rect x="{cx-30}" y="{cy+5}" width="60" height="17" rx="8.5"
        fill="{badge_bg}" stroke="{clr}" stroke-width="1"/>
  <text x="{cx}" y="{cy+17}" font-family="IBM Plex Sans,sans-serif" font-size="7.5"
        font-weight="800" fill="{clr}" text-anchor="middle" letter-spacing="0.8">{status}</text>

  <!-- Footer label -->
  <text x="{cx}" y="{H-2}" font-family="IBM Plex Mono,monospace" font-size="7"
        fill="#2d4a6a" text-anchor="middle" letter-spacing="2.5">UKC METER</text>
</svg>"""

    # ── Render via components.html to bypass Streamlit SVG sanitiser ───────
    card_border = border_clr
    html = f"""<!DOCTYPE html>
<html><head><style>
  body{{margin:0;padding:0;background:#111827;font-family:'IBM Plex Sans',sans-serif;}}
  .card{{background:#1a2236;border:1px solid {card_border};border-radius:8px;padding:14px 16px;}}
  .lbl{{font-size:11px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.1em;
  ...
  .req{{font-size:11px;color:#94a3b8;}}
  .req strong{{color:#e2e8f0;font-family:'IBM Plex Mono',monospace;}}
  .badge{{display:inline-block;padding:3px 12px;border-radius:100px;
          font-size:10px;font-weight:800;letter-spacing:0.06em;
          background:{badge_bg};color:{clr};border:1px solid {card_border};}}
</style></head>
<body>
  <div class="card">
    ...
    {svg}
    <div class="foot">
      <span class="req">Required minimum: <strong>{required:.2f} m</strong></span>
...
def extract_q88_fields(raw_text):
    ...
    for pattern, src_label in [
        (r"8\.2a\s+Grand Total.*?[:\s]+([\d,.]+)\s*Cu",       "Q88-8.2a Grand Total"),
        ...
        if q88:
            ...
            with st.expander("📋 Raw extraction results (for verification)"):
                _raw_rows = [
                    ...
                    ("98% Tank Capacity (m³)", q88.get("tank_m3_98") or "—"),
            ...
            with st.form("q88_review_form"):
                ...
                with rc2:
                    ...
                    r_tank  = st.number_input(
                        ...
                        value=float(q88.get("tank_m3_98") or 1000.0),
                        step=10.0,
                        help=f"Source: {q88.get('tank_src', 'Q88 cargo section')}. "
                             "Grand Total (wing + centre cargo tanks at 98%). "
                             "Do NOT include slop tanks unless they routinely carry cargo.",
                    )
