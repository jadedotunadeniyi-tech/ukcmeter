...
def summary_png_bytes(rows: list[tuple], title: str = "Loading Calculation Summary") -> bytes:
    """
    Render the loading summary table as a PNG image using HTML → bytes.
    Uses a simple HTML table rendered to an SVG then to PNG via the
    built-in matplotlib approach so no external binary is required.
    ...
    for (row_idx, col_idx), cell in tbl.get_celld().items():
        ...
        if row_idx == 0:                               # header
            ...
            cell.set_text_props(color="#7dd3fc", fontweight="bold", fontfamily="monospace")
            cell.set_edgecolor("#0ea5e9")
        else:
            cell.set_facecolor("#1a2236" if row_idx % 2 == 0 else "#111827")
