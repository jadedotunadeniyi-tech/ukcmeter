...
def ukc_badge(status: str, ukc: float, required: float,
    ...
    """
    Render the UKC Meter arc gauge using st.components.v1.html().
    
    st.markdown() strips <svg> via its HTML sanitiser — components.html()
    embeds a full iframe and renders SVG reliably without any sanitisation.
    """
    import math
    ...
    # ── Status colours ──────────────────────────────────────────────
    if status == "ADEQUATE":
        clr, badge_bg, border_clr = "#10b981", "rgba(16,185,129,0.18)", "rgba(16,185,129,0.4)"
    elif status == "MARGINAL":
        clr, badge_bg, border_clr = "#f59e0b", "rgba(245,158,11,0.18)", "rgba(245,158,11,0.4)"
    else:
        clr, badge_bg, border_clr = "#ef4444", "rgba(239,68,68,0.18)", "rgba(239,68,68,0.4)"
    icon = {"ADEQUATE": "✓", "MARGINAL": "⚠"}.get(status, "✕")

    # ── Needle position ──────────────────────────────────────────────
    # frac=0 → needle left (safe), frac=1 → needle right (aground)
    if depth_m and depth_m > 0 and draft_m is not None:
        frac = min(1.0, max(0.0, draft_m / depth_m))
    else:
        # Fallback: use UKC vs required as proxy
        frac = max(0.0, min(1.0, 1.0 - (ukc / max(required * 2, 0.01)))) if required > 0 else 0.3
    ...
    R_o, R_i  = 97, 65
    R_needle  = 85
    START     = 200    # leftmost angle (CCW from east)
    SWEEP     = 220    # total arc span degrees

    def _x(r, deg): return cx + r * math.cos(math.radians(deg))
    def _y(r, deg): return cy - r * math.sin(math.radians(deg))

    # Zone angles
    z_red   = START - SWEEP          # rightmost = danger
    z_amber = START - SWEEP * 0.30   # 30% from danger
    z_green = START - SWEEP * 0.60   # 60% from danger

    def _arc(r_o, r_i, a1, a2, fill):
        """Donut arc path: outer and inner radius, from angle a1 to a2."""
        large = 1 if abs(a2 - a1) > 180 else 0
        return (
            f'<path d="M {_x(r_o,a1):.2f} {_y(r_o,a1):.2f} '
            f'A {r_o} {r_o} 0 {large} 0 {_x(r_o,a2):.2f} {_y(r_o,a2):.2f} '
            f'L {_x(r_i,a2):.2f} {_y(r_i,a2):.2f} '
            f'A {r_i} {r_i} 0 {large} 1 {_x(r_i,a1):.2f} {_y(r_i,a1):.2f} Z" '
            f'fill="{fill}" />'
        )

    # Build tick marks (11 ticks along the arc)
    ticks = ""
    for i in range(11):
        ang = START - i / 10 * SWEEP
        r1  = R_o - 2
        r2  = R_o - (8 if i % 5 == 0 else 4)
        ticks += (
            f'<line x1="{_x(r1,ang):.2f}" y1="{_y(r1,ang):.2f}" '
    ...
    ntx, nty   = _x(R_needle, n_ang),    _y(R_needle, n_ang)
    nb1x, nb1y = _x(7, n_ang + 90),      _y(7, n_ang + 90)
    nb2x, nb2y = _x(7, n_ang - 90),      _y(7, n_ang - 90)
  ...
  <circle cx="{cx}" cy="{cy}" r="{R_o+10}" fill="url(#bg)" stroke="#1a3a5c" stroke-width="1"/>

  {_arc(R_o, R_i, z_red, START, "#0f1e30")}
  {_arc(R_o-1, R_i+1, z_red,   z_amber, "rgba(239,68,68,0.28)")}
  {_arc(R_o-1, R_i+1, z_amber, z_green, "rgba(245,158,11,0.24)")}
  {_arc(R_o-1, R_i+1, z_green, START,   "rgba(16,185,129,0.24)")}

  <line x1="{_x(R_i,z_amber):.2f}" y1="{_y(R_i,z_amber):.2f}" x2="{_x(R_o,z_amber):.2f}" y2="{_y(R_o,z_amber):.2f}" stroke="#1a3a5c" stroke-width="1.5"/>
  <line x1="{_x(R_i,z_green):.2f}" y1="{_y(R_i,z_green):.2f}" x2="{_x(R_o,z_green):.2f}" y2="{_y(R_o,z_green):.2f}" stroke="#1a3a5c" stroke-width="1.5"/>

  {ticks}

  <text x="{_x(R_o+15, START-7):.2f}" y="{_y(R_o+15, START-7):.2f}" font-family="monospace" font-size="8" fill="#10b981" font-weight="700" text-anchor="middle">SAFE</text>
  <text x="{_x(R_o+15, z_red+7):.2f}" y="{_y(R_o+15, z_red+7):.2f}" font-family="monospace" font-size="8" fill="#ef4444" font-weight="700" text-anchor="middle">RISK</text>

  <polygon points="{nb1x:.2f},{nb1y:.2f} {nb2x:.2f},{nb2y:.2f} {ntx:.2f},{nty:.2f}" fill="rgba(0,0,0,0.4)" transform="translate(2,2)"/>
  <polygon points="{nb1x:.2f},{nb1y:.2f} {nb2x:.2f},{nb2y:.2f} {ntx:.2f},{nty:.2f}" fill="{clr}" filter="url(#glow)"/>

  <circle cx="{cx}" cy="{cy}" r="10" fill="#0b1220" stroke="{clr}" stroke-width="2.5"/>
  <circle cx="{cx}" cy="{cy}" r="4.5" fill="{clr}"/>

  <text x="{cx}" y="{cy-27}" font-family="monospace" font-size="22" font-weight="700" fill="{clr}" text-anchor="middle" filter="url(#glow)">{ukc:.2f}</text>
  <text x="{cx}" y="{cy-12}" font-family="sans-serif" font-size="8.5" fill="#94a3b8" text-anchor="middle">metres UKC</text>

  <rect x="{cx-32}" y="{cy+5}" width="64" height="18" rx="9" fill="{badge_bg}" stroke="{clr}" stroke-width="1"/>
  <text x="{cx}" y="{cy+18}" font-family="sans-serif" font-size="8" font-weight="800" fill="{clr}" text-anchor="middle" letter-spacing="0.8">{status}</text>

  <text x="{cx}" y="{H-1}" font-family="monospace" font-size="7" fill="#2d4a6a" text-anchor="middle" letter-spacing="2.5">UKC METER</text>
</svg>"""

    html = f"""<!DOCTYPE html><html><head><style>
*{{margin:0;padding:0;box-sizing:border-box;}}
body{{background:#111827;font-family:system-ui,sans-serif;padding:12px;}}
.card{{background:#1a2236;border:1px solid {border_clr};border-radius:10px;padding:14px 16px;}}
.lbl{{font-size:10.5px;color:#94a3b8;text-transform:uppercase;letter-spacing:.1em;
      font-family:monospace;margin-bottom:10px;font-weight:600;}}
.foot{{display:flex;justify-content:space-between;align-items:center;
       margin-top:10px;padding-top:10px;border-top:1px solid #1a3a5c;}}
.req{{font-size:11px;color:#94a3b8;}}
.req strong{{color:#e2e8f0;font-family:monospace;}}
.badge{{padding:4px 14px;border-radius:100px;font-size:10px;font-weight:800;
        letter-spacing:.08em;background:{badge_bg};color:{clr};border:1px solid {border_clr};}}
</style></head><body>
<div class="card">
  ...
  <div class="foot">
    ...
    _comp.html(html, height=290, scrolling=False)
