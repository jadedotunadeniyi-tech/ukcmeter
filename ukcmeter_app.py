...
_BASE_VESSELS: list[dict] = [
    ...
    {"name":"MT Bedford","class":"General Purpose",
     ...
     "constant":200,"bunker_fw":1500,"draft_full":7.146,"block_coeff":0.620,
    ...
    {"name":"MT Bagshot","class":"General Purpose",
     ...
     "constant":29,"bunker_fw":1500,"draft_full":9.1,"block_coeff":0.720,
    ...
    {"name":"MT Woodstock","class":"General Purpose",
     ...
     "constant":250,"bunker_fw":1500,"draft_full":8.252,"block_coeff":0.760,
    ...
    {"name":"MT Rathbone","class":"General Purpose",
     ...
     "constant":220,"bunker_fw":1500,"draft_full":6.956,"block_coeff":0.720,
    ...
    {"name":"MT Rahama","class":"General Purpose",
     ...
     "constant":0,"bunker_fw":1500,"draft_full":6.14,"block_coeff":0.700,
    ...
    {"name":"MT Enford","class":"General Purpose",
     ...
     "constant":250,"bunker_fw":1500,"draft_full":8.983,"block_coeff":0.740,
     "breakwater_lat":3.45,"api_ref":28.9,"note":""},
    ...
    {"name":"Duke","class":"General Purpose",
     ...
     "constant":200,"bunker_fw":1500,"draft_full":8.5,"block_coeff":0.720,
...
def volume_to_draft(v: dict, volume_m3: float, api: float,
    ...
    # Draft in reference seawater (vessel hydrostatics calibrated at SEAWATER_DENSITY_NG)
    draft_ref   = _draft_from_displacement(v, total_disp)

    # Density correction — FWA per IACS UR L4:
    # draft in actual water = draft_ref × (ρ_ref / ρ_actual)
    # Lighter water → deeper draft; denser water → shallower draft.
    rho_ref     = SEAWATER_DENSITY_NG
...
def draft_to_volume(v: dict, draft_m: float, api: float,
    ...
    # Convert input draft to reference seawater equivalent (undo FWA)
    rho_ref     = SEAWATER_DENSITY_NG
    density_factor = rho_ref / max(sw_density, 0.99)
    draft_ref   = draft_m / density_factor   # = draft_m × (ρ_actual / ρ_ref)
...
def _ukc_svg(status, ukc, required, depth_m=None, draft_m=None, marginal_m=None, pct_adq=0.15, pct_mrg=0.05):
    ...
    parts = [
        ...
        ' fill="#30D158" text-anchor="middle" opacity="0.9">' + str(int(pct_adq*100)) + '%</text>',
        f'<text x="{lx_mrg:.2f}" y="{ly_mrg-7:.2f}"',
        ' font-family="ui-monospace,monospace" font-size="9" font-weight="700"',
        ' fill="#FF9F0A" text-anchor="middle" opacity="0.9">' + str(int(pct_mrg*100)) + '%</text>',
