...
def _ukc_svg(status: str, ukc: float, required: float,
             depth_m: float = None, draft_m: float = None) -> str:
    """
    Return the raw SVG string for the UKC gauge, scaled to fit inside the hero card.
    VW is 100% width; VH is proportionally shorter so it sits cleanly in the gap.
    """
    import math
    ...
    for i in range(10):
        ang   = i * (SWEEP / 9)
        major = (i % 3 == 0)
        r1, r2 = R_o + 2, R_o + (14 if major else 7)
        col = "rgba(255,255,255,0.3)" if major else "rgba(255,255,255,0.12)"
    ...
    # Label positions — all below the 180° baseline
    val_y   = cy + 34
    unit_y  = cy + 58
    badge_y = cy + 84

    return f"""<svg viewBox="0 0 {VW} {VH}" xmlns="http://www.w3.org/2000/svg"
  style="width:100%;display:block;max-height:260px">
  <defs>
    <radialGradient id="ibg" cx="50%" cy="80%" r="70%">
  ...
  <circle cx="{cx}" cy="{cy}" r="{R_o+18}" fill="url(#ibg)"/>
  ...
  <text x="{cx}" y="{VH-4}"
        font-family="ui-monospace,monospace" font-size="8" font-weight="500"
        fill="rgba(255,255,255,0.12)" text-anchor="middle" letter-spacing="3">UKC METER</text>
</svg>"""
...
with tab1:
    ...
    with col_out:
        ...
        # ── Unified output card (Apple-style) ─────────────────────────────
        st.markdown(f"""
        <div style="
          background:var(--surface2);
          border:1px solid var(--border2);
        ...
        ">
          <div style="position:absolute;top:0;left:0;right:0;height:1px;
            background:linear-gradient(90deg,transparent 5%,{dc} 40%,{ukc_clr} 60%,transparent 95%)">
          </div>
          ...
          <div style="display:flex;align-items:baseline;gap:0.5rem;margin-bottom:0.2rem">
            <span style="font-size:3.8rem;font-weight:700;font-family:var(--mono);
              color:{dc};letter-spacing:-0.04em;line-height:1">{res['draft_m']:.3f}</span>
            <span style="font-size:1.6rem;font-weight:400;color:var(--text-dim)">m</span>
            {"<span style=\"font-size:0.75rem;color:#ff453a;font-weight:600;margin-left:0.5rem\">" + ol + "</span>" if ol else ""}
          </div>
          <div style="font-size:0.8rem;color:var(--text-dim);margin-bottom:0.75rem">
            TLD limit&nbsp;{res['tld_m']} m &nbsp;&middot;&nbsp; UKC headroom&nbsp;
            <span style="color:{ukc_clr};font-weight:600">{round(water_depth - res['draft_m'],2):.2f} m</span>
          </div>

          <!-- UKC Gauge embedded in the free space -->
          <div style="margin:0 -0.5rem 0.75rem;border-top:1px solid rgba(255,255,255,0.06);
            ...
            {_ukc_svg(ukc["ukc_status"], ukc["ukc_m"], ukc["ukc_required_m"], water_depth, res["draft_m"])}
            <div style="display:flex;justify-content:space-between;align-items:center;
              padding:4px 8px 6px;font-size:11px;color:rgba(255,255,255,0.4)">
              <span>Min required: <strong style="color:rgba(255,255,255,0.75);
                font-family:ui-monospace,monospace">{ukc["ukc_required_m"]:.2f} m</strong></span>
            </div>
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;
            ...
            <div style="text-align:center">
              ...
              <div style="font-size:1.15rem;font-weight:700;font-family:var(--mono);
                color:var(--text)">{res['volume_loaded_bbl']:,}</div>
              <div style="font-size:0.65rem;color:var(--text-muted)">US Bbls</div>
            </div>
            <div style="text-align:center;border-left:1px solid var(--border);border-right:1px solid var(--border)">
              <div style="font-size:0.55rem;font-weight:700;letter-spacing:0.14em;
                text-transform:uppercase;color:var(--text-muted);margin-bottom:0.3rem">Volume</div>
              <div style="font-size:1.15rem;font-weight:700;font-family:var(--mono);
                color:var(--text)">{res['volume_loaded_m3']:,.1f}</div>
              <div style="font-size:0.65rem;color:var(--text-muted)">m³</div>
            </div>
            <div style="text-align:center">
              <div style="font-size:0.55rem;font-weight:700;letter-spacing:0.14em;
                text-transform:uppercase;color:var(--text-muted);margin-bottom:0.3rem">Cargo Mass</div>
              <div style="font-size:1.15rem;font-weight:700;font-family:var(--mono);
                color:var(--text)">{res['cargo_mass_mt']:,.0f}</div>
              <div style="font-size:0.65rem;color:var(--text-muted)">MT</div>
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)
...
with tab2:
    ...
    with col_out2:
        ...
        ">
          <div style="position:absolute;top:0;left:0;right:0;height:1px;
            background:linear-gradient(90deg,transparent 5%,#0a84ff 40%,{ukc_clr2} 60%,transparent 95%)">
          </div>
          <div style="font-size:0.62rem;font-weight:700;letter-spacing:0.14em;
            text-transform:uppercase;color:var(--text-muted);margin-bottom:0.4rem">
            Cargo Volume at {draft_inp:.3f} m draft
          </div>
          <div style="display:flex;align-items:baseline;gap:0.5rem;margin-bottom:0.2rem">
            <span style="font-size:3.8rem;font-weight:700;font-family:var(--mono);
              color:#0a84ff;letter-spacing:-0.04em;line-height:1">{res2['volume_bbl']:,}</span>
            <span style="font-size:1.4rem;font-weight:400;color:var(--text-dim);margin-left:4px">US Bbls</span>
          </div>
          <div style="font-size:0.8rem;color:var(--text-dim);margin-bottom:0.75rem">
            UKC at draft&nbsp;
            <span style="color:{ukc_clr2};font-weight:600">{ukc2['ukc_m']:.2f} m</span>
            &nbsp;&middot;&nbsp; TLD limit&nbsp;{res2['tld_m']} m
          </div>

          <!-- UKC Gauge embedded in the free space -->
          <div style="margin:0 -0.5rem 0.75rem;border-top:1px solid rgba(255,255,255,0.06);
            border-bottom:1px solid rgba(255,255,255,0.06);padding:0.25rem 0">
            {_ukc_svg(ukc2["ukc_status"], ukc2["ukc_m"], ukc2["ukc_required_m"], water_depth, draft_inp)}
            <div style="display:flex;justify-content:space-between;align-items:center;
              padding:4px 8px 6px;font-size:11px;color:rgba(255,255,255,0.4)">
              <span>Min required: <strong style="color:rgba(255,255,255,0.75);
                font-family:ui-monospace,monospace">{ukc2["ukc_required_m"]:.2f} m</strong></span>
            </div>
          </div>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;
            ...
            <div style="text-align:center">
              <div style="font-size:0.55rem;font-weight:700;letter-spacing:0.14em;
                text-transform:uppercase;color:var(--text-muted);margin-bottom:0.3rem">Volume</div>
              <div style="font-size:1.15rem;font-weight:700;font-family:var(--mono);
                color:var(--text)">{res2['volume_m3']:,.1f}</div>
              <div style="font-size:0.65rem;color:var(--text-muted)">m³</div>
            </div>
            <div style="text-align:center;border-left:1px solid var(--border);border-right:1px solid var(--border)">
              <div style="font-size:0.55rem;font-weight:700;letter-spacing:0.14em;
                text-transform:uppercase;color:var(--text-muted);margin-bottom:0.3rem">Cargo Mass</div>
              <div style="font-size:1.15rem;font-weight:700;font-family:var(--mono);
                color:var(--text)">{res2['cargo_mass_mt']:,.0f}</div>
              <div style="font-size:0.65rem;color:var(--text-muted)">MT</div>
            </div>
            <div style="text-align:center">
              <div style="font-size:0.55rem;font-weight:700;letter-spacing:0.14em;
                text-transform:uppercase;color:var(--text-muted);margin-bottom:0.3rem">SG @ {api:.1f}\u00b0</div>
              <div style="font-size:1.15rem;font-weight:700;font-family:var(--mono);
                color:var(--text)">{res2['sg']:.4f}</div>
              <div style="font-size:0.65rem;color:var(--text-muted)">t/m\u00b3</div>
            </div>
          </div>
        </div>
        """, unsafe_allow_html=True)
