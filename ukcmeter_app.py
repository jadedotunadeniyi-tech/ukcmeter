...
# Thresholds as % of laden draft — validated by BV & DNV class surveyors
#   ADEQUATE  ≥ 15% draft  (PIANC 2014 open-water minimum for loaded tankers)
#   MARGINAL   5–15% draft  (alert zone — restricted channel/bar approach)
#   AGROUND    < 5% draft   (squat + wave action consume remaining margin)
# Absolute floors (NNPC/NPA / IMO — never go below regardless of %):
UKC_PCT_ADEQUATE     = 0.15     # 15% of laden draft → ADEQUATE threshold
UKC_PCT_MARGINAL     = 0.05     # 5%  of laden draft → MARGINAL / AGROUND boundary
UKC_MIN_RIVER        = 0.30     # m — restricted waterway absolute minimum (NNPC/NPA)
UKC_MIN_OPEN         = 0.50     # m — open water absolute minimum (IMO)
# Mother vessel classes (Suezmax / Aframax) operate at BIA offshore — density locked
...
def ukc_assessment(draft_m: float, water_depth: float) -> dict:
    ...
    ADEQUATE  : UKC ≥ 15% of draft  (and ≥ UKC_MIN_OPEN absolute floor)
    MARGINAL  : UKC ≥  5% of draft  (and ≥ UKC_MIN_RIVER absolute floor)
    AGROUND   : UKC <  5% of draft  OR UKC ≤ 0

    Returns both the ADEQUATE threshold (gauge full-scale reference)
    and the MARGINAL threshold (amber/red boundary on the gauge arc).
    """
    ukc        = water_depth - draft_m
    thresh_adq = max(UKC_MIN_OPEN,  draft_m * UKC_PCT_ADEQUATE)  # 15% draft
    thresh_mrg = max(UKC_MIN_RIVER, draft_m * UKC_PCT_MARGINAL)  # 5%  draft
    if ukc >= thresh_adq:
        status = "ADEQUATE"
    elif ukc >= thresh_mrg:
        status = "MARGINAL"
    ...
    return {
        "ukc_m":          round(ukc, 2),
        "ukc_required_m": round(thresh_adq, 2),   # ADEQUATE threshold = gauge full-scale
        "marginal_m":     round(thresh_mrg, 2),    # MARGINAL threshold = amber/red boundary
        "ukc_status":     status,
    }
...
def _ukc_svg(status, ukc, required, depth_m=None, draft_m=None, marginal_m=None):
    """
    Calibrated UKC gauge — IMO MSC/Circ.1418 · PIANC 2014 · BV/DNV-GL.

    Arc zones anchored to real thresholds (NOT equal thirds):
      GREEN  0° → 90°       UKC ≥ ADEQUATE threshold (15% draft)
      AMBER  90° → ~150°    MARGINAL zone (5–15% draft)
      RED    ~150° → 180°   AGROUND RISK (< 5% draft)
    ...
    ang_mrg = 180.0 * max(0.01, 1.0 - thresh_mrg / full_scale)
    ...
    def _arc(ro, ri, a1, a2, fill):
        if abs(a2 - a1) < 0.3: return ""
        lg = 1 if abs(a2 - a1) > 180 else 0
        p = (f"M {_x(ro,a1):.2f} {_y(ro,a1):.2f} "
             f"A {ro} {ro} 0 {lg} 1 {_x(ro,a2):.2f} {_y(ro,a2):.2f} "
             f"L {_x(ri,a2):.2f} {_y(ri,a2):.2f} "
             f"A {ri} {ri} 0 {lg} 0 {_x(ri,a1):.2f} {_y(ri,a1):.2f} Z")
        return f'<path d="{p}" fill="{fill}"/>'

    # Ticks every 10° — highlighted at threshold boundaries
    ticks = ""
    for deg in range(0, 181, 10):
        at_adq = abs(deg - ang_adq) < 8
        at_mrg = abs(deg - ang_mrg) < 8
        major  = (deg % 30 == 0) or at_adq or at_mrg
        r1 = R_o + 2
        r2 = R_o + (17 if (at_adq or at_mrg) else 13 if major else 6)
        col = ("#30D158" if at_adq else "#FF9F0A" if at_mrg
               else "rgba(255,255,255,0.32)" if major else "rgba(255,255,255,0.09)")
        sw  = "2.5" if (at_adq or at_mrg) else "2" if major else "1"
    ...
    # Threshold % label positions on arc
    lx_adq = _x(R_o + 28, ang_adq); ly_adq = _y(R_o + 28, ang_adq)
    lx_mrg = _x(R_o + 28, ang_mrg); ly_mrg = _y(R_o + 28, ang_mrg)

    parts = [
        f'<svg viewBox="0 0 {VW} {VH}" xmlns="http://www.w3.org/2000/svg"',
        ' style="width:100%;display:block;max-height:270px">',
        '<defs>',
        ...
        '<feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>',
        '</defs>',
        f'<circle cx="{cx}" cy="{cy}" r="{R_o+20}" fill="url(#ibg2)"/>',
        _arc(R_o, R_i, 0,       ang_adq, "#30D158"),
        _arc(R_o, R_i, ang_adq, ang_mrg, "#FF9F0A"),
        _arc(R_o, R_i, ang_mrg, 180,     "#FF453A"),
        f'<line x1="{_x(R_i,ang_adq):.2f}" y1="{_y(R_i,ang_adq):.2f}"',
        f' x2="{_x(R_o,ang_adq):.2f}" y2="{_y(R_o,ang_adq):.2f}" stroke="#070707" stroke-width="3"/>',
        f'<line x1="{_x(R_i,ang_mrg):.2f}" y1="{_y(R_i,ang_mrg):.2f}"',
        f' x2="{_x(R_o,ang_mrg):.2f}" y2="{_y(R_o,ang_mrg):.2f}" stroke="#070707" stroke-width="3"/>',
        ...
        ticks,
        f'<text x="{_x(R_o+36,0):.2f}" y="{_y(R_o+36,0)+5:.2f}"',
        ' font-family="system-ui,sans-serif" font-size="12" font-weight="800"',
        ' fill="#30D158" text-anchor="middle">SAFE</text>',
        f'<text x="{_x(R_o+36,180):.2f}" y="{_y(R_o+36,180)+5:.2f}"',
        ' font-family="system-ui,sans-serif" font-size="12" font-weight="800"',
        ' fill="#FF453A" text-anchor="middle">RISK</text>',
        # Threshold % annotations at arc boundaries
        f'<text x="{lx_adq:.2f}" y="{ly_adq-7:.2f}"',
        ' font-family="ui-monospace,monospace" font-size="9" font-weight="700"',
        ' fill="#30D158" text-anchor="middle" opacity="0.9">15%</text>',
        f'<text x="{lx_mrg:.2f}" y="{ly_mrg-7:.2f}"',
        ' font-family="ui-monospace,monospace" font-size="9" font-weight="700"',
        ' fill="#FF9F0A" text-anchor="middle" opacity="0.9">5%</text>',
...
with st.sidebar:
    ...
    if _is_mother:
        ...
        _SW_PRESETS = {
            ...
            "Fresh water — terminal / river berth":             1.0000,
        }
        ...
        if _sw_preset_val is not None:
            ...
            _sw_default = SEAWATER_DENSITY_NG
...
with tab1:
    ...
    with col_out:
        ...
        _card_gauge = (
            ...
            'font-family:ui-monospace,monospace">' + _ukc_req + ' m</strong></span></div></div>'
        )
        _card_bottom = (
...
with tab2:
    ...
    with col_out2:
        ...
        _c2_gauge = (
            ...
            '<span>Min required: <strong style="color:rgba(255,255,255,0.75);' +
            'font-family:ui-monospace,monospace">' + _ukc_req2 + ' m</strong></span></div></div>'
        )
        _c2_bottom = (
            '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;' +
            'padding:1rem 0;border-top:1px solid var(--border);border-bottom:1px solid var(--border);' +
