...
def ukc_meter_svg(ukc_m: float, required_m: float, status: str,
                  depth_m: float = None, draft_m: float = None) -> str:
    """
    Render the signature ukcmeter arc gauge as an SVG.

    The arc sweeps 220° (from ~160° to ~20° clockwise, with 0° = right).
    Red zone = 0–30%, Amber = 30–60%, Green = 60–100%.
    Needle points to the current UKC as a fraction of water depth.
    """
    import math

    # Fraction of depth used by draft  (needle: 0=shallow draft, 1=aground)
    if depth_m and depth_m > 0:
        frac = min(1.0, max(0.0, draft_m / depth_m)) if draft_m else 0.0
    else:
        frac = 0.5
    ...
    R_inner  = 72
    R_needle = 95
    START_ANG = 200   # degrees, CCW from east (SVG: CW from east)
    SWEEP     = 220   # degrees total

    def polar(r, deg):
        rad = math.radians(deg)
        return cx + r * math.cos(rad), cy - r * math.sin(rad)

    def arc_path(r, a1, a2, large=0):
        x1, y1 = polar(r, a1)
        x2, y2 = polar(r, a2)
        return f"M {x1:.1f} {y1:.1f} A {r} {r} 0 {large} 0 {x2:.1f} {y2:.1f}"

    # Colour zones (in degrees along the sweep, CCW)
    def zone_path(r_o, r_i, ang_start, ang_end):
        large = 1 if abs(ang_end - ang_start) > 180 else 0
        o1x, o1y = polar(r_o, ang_start)
        o2x, o2y = polar(r_o, ang_end)
        i1x, i1y = polar(r_i, ang_start)
        i2x, i2y = polar(r_i, ang_end)
        return (f"M {o1x:.1f} {o1y:.1f} "
                f"A {r_o} {r_o} 0 {large} 0 {o2x:.1f} {o2y:.1f} "
                f"L {i2x:.1f} {i2y:.1f} "
                f"A {r_i} {r_i} 0 {large} 1 {i1x:.1f} {i1y:.1f} Z")

    # Map 0→1 fraction to degrees (START_ANG down to START_ANG-SWEEP, i.e. decreasing angle)
    # 0 = safe (left side), 1 = aground (right side)
    ang_safe   = START_ANG                        # 200°
    ang_danger = START_ANG - SWEEP                # -20° = 340°

    # Zone boundaries
    z30 = START_ANG - SWEEP * 0.30   # 30% point (amber)
    z60 = START_ANG - SWEEP * 0.60   # 60% point (green)

    # Needle angle — higher frac = needle sweeps toward danger
    needle_ang = START_ANG - frac * SWEEP
    nx, ny = polar(R_needle, needle_ang)
    nbase1x, nbase1y = polar(8, needle_ang + 90)
    nbase2x, nbase2y = polar(8, needle_ang - 90)
...
<svg viewBox="0 0 {W} {H}" xmlns="http://www.w3.org/2000/svg"
     style="width:100%;max-width:260px;display:block;margin:0 auto;overflow:visible">
  <defs>
    <radialGradient id="bg-grad" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stop-color="#111f35"/>
  ...
  <!-- Track background -->
  <path d="{zone_path(R_outer, R_inner, ang_danger, ang_safe)}"
        fill="#0f1e30" stroke="#1a3a5c" stroke-width="0.5"/>

  <!-- RED zone: 70-100% draft fraction (danger) -->
  <path d="{zone_path(R_outer-1, R_inner+1, ang_danger, z30)}"
        fill="rgba(239,68,68,0.22)" stroke="none"/>

  <!-- AMBER zone: 40-70% -->
  <path d="{zone_path(R_outer-1, R_inner+1, z30, z60)}"
        fill="rgba(245,158,11,0.18)" stroke="none"/>

  <!-- GREEN zone: 0-40% draft fraction (safe) -->
  <path d="{zone_path(R_outer-1, R_inner+1, z60, ang_safe)}"
        fill="rgba(16,185,129,0.18)" stroke="none"/>

  <!-- Zone arc borders -->
  <line x1="{polar(R_inner,z30)[0]:.1f}" y1="{polar(R_inner,z30)[1]:.1f}"
        x2="{polar(R_outer,z30)[0]:.1f}" y2="{polar(R_outer,z30)[1]:.1f}"
        stroke="#1a3a5c" stroke-width="1.5"/>
  <line x1="{polar(R_inner,z60)[0]:.1f}" y1="{polar(R_inner,z60)[1]:.1f}"
        x2="{polar(R_outer,z60)[0]:.1f}" y2="{polar(R_outer,z60)[1]:.1f}"
        stroke="#1a3a5c" stroke-width="1.5"/>

  <!-- Tick marks -->
  {''.join(
    f'<line x1="{polar(R_outer-2, START_ANG - i/10*SWEEP)[0]:.1f}" '
    f'y1="{polar(R_outer-2, START_ANG - i/10*SWEEP)[1]:.1f}" '
    f'x2="{polar(R_outer-8 if i%5==0 else R_outer-5, START_ANG - i/10*SWEEP)[0]:.1f}" '
    f'y2="{polar(R_outer-8 if i%5==0 else R_outer-5, START_ANG - i/10*SWEEP)[1]:.1f}" '
    f'stroke="#2d4a6a" stroke-width="1.5"/>'
    for i in range(11)
  )}

  <!-- Zone label: SAFE -->
  <text x="{polar(R_outer+16, START_ANG-10)[0]:.1f}" y="{polar(R_outer+16, START_ANG-10)[1]:.1f}"
        font-family="Inter,sans-serif" font-size="7" fill="#10b981" font-weight="700"
        text-anchor="middle">SAFE</text>

  <!-- Zone label: RISK -->
  <text x="{polar(R_outer+16, ang_danger+10)[0]:.1f}" y="{polar(R_outer+16, ang_danger+10)[1]:.1f}"
        font-family="Inter,sans-serif" font-size="7" fill="#ef4444" font-weight="700"
        text-anchor="middle">RISK</text>

  <!-- Needle shadow -->
  <polygon points="{nbase1x:.1f},{nbase1y:.1f} {nbase2x:.1f},{nbase2y:.1f} {nx:.1f},{ny:.1f}"
           fill="rgba(0,0,0,0.4)" transform="translate(2,2)"/>
  ...
  <!-- Hub -->
  <circle cx="{cx}" cy="{cy}" r="9" fill="#0d1829" stroke="{clr}" stroke-width="2"/>
  <circle cx="{cx}" cy="{cy}" r="4" fill="{clr}"/>

  <!-- UKC value -->
  <text x="{cx}" y="{cy - 28}" font-family="JetBrains Mono,monospace" font-size="22"
        font-weight="700" fill="{ukc_text_clr}" text-anchor="middle"
        filter="url(#glow)">{ukc_m:.2f}</text>
  <text x="{cx}" y="{cy - 13}" font-family="Inter,sans-serif" font-size="8"
        fill="#94a3b8" text-anchor="middle">metres UKC</text>

  <!-- Status label -->
  <rect x="{cx-28}" y="{cy+6}" width="56" height="16" rx="8"
        fill="{clr}" fill-opacity="0.18" stroke="{clr}" stroke-width="1"/>
  <text x="{cx}" y="{cy+18}" font-family="Inter,sans-serif" font-size="7.5"
        font-weight="800" fill="{clr}" text-anchor="middle" letter-spacing="0.5">{status}</text>

  <!-- "UKC METER" label -->
  <text x="{cx}" y="{H - 4}" font-family="JetBrains Mono,monospace" font-size="7"
        fill="#4a6580" text-anchor="middle" letter-spacing="2">UKC METER</text>
</svg>"""
    return svg
...
def ukc_badge(status: str, ukc: float, required: float,
    ...
    """Render the UKC meter SVG + metric card below it."""
    clr  = {"ADEQUATE": "#10b981", "MARGINAL": "#f59e0b"}.get(status, "#ef4444")
    icon = {"ADEQUATE": "✓", "MARGINAL": "⚠"}.get(status, "✕")
    cls  = {"ADEQUATE": "badge-ok", "MARGINAL": "badge-warn"}.get(status, "badge-risk")

    svg_html = ukc_meter_svg(ukc, required, status, depth_m, draft_m)

    st.markdown(f"""
    <div class="ukc-card" style="border-color:{'rgba(16,185,129,0.3)' if status=='ADEQUATE' else ('rgba(245,158,11,0.3)' if status=='MARGINAL' else 'rgba(239,68,68,0.4)')}">
      <div class="metric-label" style="margin-bottom:0.5rem">⚓ UNDER KEEL CLEARANCE (UKC)</div>
      {svg_html}
      <div style="display:flex;align-items:center;justify-content:space-between;margin-top:0.5rem;padding-top:0.5rem;border-top:1px solid var(--border)">
        <span style="font-size:0.75rem;color:var(--text-dim);font-family:var(--sans)">
          Required minimum: <strong style="color:var(--text);font-family:var(--mono)">{required:.2f} m</strong>
        </span>
