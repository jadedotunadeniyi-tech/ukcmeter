...
def _ukc_svg(status, ukc, required, depth_m=None, draft_m=None, marginal_m=None, pct_adq=0.15, pct_mrg=0.05):
    ...
    Arc zones anchored to real thresholds (NOT equal thirds):
      GREEN  0° → ang_adq   UKC ≥ ADEQUATE threshold (e.g. 20%/15% draft)
    ...
    Arc rendering fix: we draw from -0.5° to 180.5° to ensure the arc completely
    fills the semicircle at both ends — no rendering gap at the endcaps.
    """
    import math
    ...
    ang_adq = 180.0 * (1.0 - thresh_adq / full_scale)        # e.g. 90° for 15%/15%
    ...
    def _arc(ro, ri, a1, a2, fill):
        """Draw arc from a1→a2 (both in 0–180° range, 0=right, 180=left).
        Slight overdraw: extend 0.5° past both ends to prevent rendering gap at endcaps."""
    ...
    # ── Tick marks ──────────────────────────────────────────────────────────────
    # Place precise threshold ticks at ang_adq and ang_mrg,
    # plus regular ticks every 10°. Labels positioned to avoid overlaps.
    ticks = ""
    # Regular ticks at every 10°
    for deg in range(0, 181, 10):
        at_adq = abs(deg - ang_adq) < 6
        at_mrg = abs(deg - ang_mrg) < 6
        major  = (deg % 30 == 0)
        if at_adq or at_mrg:
            continue   # skip — dedicated threshold ticks drawn separately below
        r1 = R_o + 2
    ...
    # Dedicated threshold ticks — longer and brightly coloured
    ...
    # ── Threshold % labels — placed just outside the arc, avoiding endcap overlap ─
    # At ang_adq = 90° the label sits directly above the arc centre.
    # Use offset outward at 45° from the boundary angle to clear the divider line.
    def _label_pos(ang, r_base, offset_deg=0):
        """Place label at angle (ang + offset_deg) at radius r_base."""
        return _x(r_base, ang + offset_deg), _y(r_base, ang + offset_deg)

    # ADEQUATE label (green) — sits on green side, offset slightly clockwise (lower angle)
    lx_adq, ly_adq = _label_pos(ang_adq, R_o + 30, -14)
    # MARGINAL label (amber) — sits on amber side, offset slightly counterclockwise (higher angle)
    lx_mrg, ly_mrg = _label_pos(ang_mrg, R_o + 30, +12)
    ...
    parts = [
        ...
        '<feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>',
        '</defs>',
        # Background disc — slightly larger to prevent edge clipping
        f'<circle cx="{cx}" cy="{cy}" r="{R_o+22}" fill="url(#ibg2)"/>',
        # ── Colour arc zones — green starts at 0°, red ends at 180° ────────────
        _arc(R_o, R_i, 0,       ang_adq, "#30D158"),
        _arc(R_o, R_i, ang_adq, ang_mrg, "#FF9F0A"),
        _arc(R_o, R_i, ang_mrg, 180,     "#FF453A"),
        # Zone dividers (drawn on top of arcs, hair-width so they don't eat arc paint)
        ...
        # Threshold % labels — placed with angular offset so they clear divider lines
        # Pill/badge background for better readability
