...
.vessel-pill {
  background: linear-gradient(135deg, rgba(10,132,255,0.22), rgba(48,209,88,0.15));
  border: 2px solid rgba(10,132,255,0.55);
  border-radius: 980px;
  padding: 10px 28px;
  font-size: 2.4rem; font-weight: 800;
  font-family: var(--mono); color: #ffffff;
  letter-spacing: -0.02em;
  box-shadow: 0 0 18px rgba(10,132,255,0.35), inset 0 1px 0 rgba(255,255,255,0.1);
...
def ukc_badge(status: str, ukc: float, required: float,
    ...
    """
    UKC Meter gauge — rendered via st.html() to bypass Streamlit HTML sanitiser.
...
def section(title: str, help_text: str = "") -> None:
    """Render a section heading. If help_text is provided, a hoverable ⓘ tooltip is added."""
    if help_text:
        # Escape any single quotes in the help text for the HTML attribute
        _esc = help_text.replace('"', '&quot;').replace("'", "&#39;")
        _tip_html = (
            f'<h2 style="display:flex;align-items:center;gap:0.45rem">'
            f'{title}'
            f'<span title="{_esc}" style="'
            f'display:inline-flex;align-items:center;justify-content:center;'
            f'width:18px;height:18px;border-radius:50%;'
            f'background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.18);'
            f'font-size:0.65rem;font-weight:700;color:rgba(255,255,255,0.55);'
            f'font-family:system-ui,sans-serif;cursor:help;flex-shrink:0;'
            f'line-height:1;margin-bottom:1px" aria-label="Help">'
            f'?</span>'
            f'</h2>'
        )
...
def extract_q88_fields(raw_text):
    ...
    # ── FWA and TPC (Q88 field 1.40) ─────────────────────────────────────────
    # Format: "1.40 FWA/TPC at summer draft: 194.00 Millimetres 29.73 Metric Tonnes"
    # or:     "1.40 FWA/TPC at summer draft: 156.00 mm 39.22 MT"
    fwa_mm = tpc_mt_cm = None
    m_fwa = srch(
        r"1\.40\s+FWA.*?:\s*([\d,.]+)\s*(?:Millimetres?|mm)\s+"
        r"([\d,.]+)\s*(?:Metric\s+Tonnes?|MT|mt)"
    )
    if m_fwa:
        fwa_mm    = _q88_num(m_fwa.group(1))
        tpc_mt_cm = _q88_num(m_fwa.group(2))
    else:
        # Some Q88s put TPC first then FWA on the same line
        m_fwa2 = srch(
            r"FWA.*?([\d,.]+)\s*(?:mm|Millimetres?).*?TPC.*?([\d,.]+)\s*(?:MT|Metric\s+Tonnes?|mt)"
        )
        if m_fwa2:
            fwa_mm    = _q88_num(m_fwa2.group(1))
            tpc_mt_cm = _q88_num(m_fwa2.group(2))
...
with tab1:
    section("Enter Cargo Volume \u2192 Predicted Draft",
            help_text=(
                ...
                "TPC-anchor method (IACS UR S11.4 / BV NR467 / DNV-GL OTG-09): "
                "displacement = Tropical displacement + TPC x 100 x (draft - TLD). "
                "Dock Water Allowance applied via certified FWA from Q88 (ILL 1966 / IACS UR L4). "
                f"Seawater density: {sw_density:.4f} t/m\u00b3. "
                "Both loading limits enforced simultaneously: "
    ...
    with col_out:
        ...
        _card_top = (
            ...
            f'text-transform:uppercase;color:var(--text-muted);margin-bottom:0.4rem">' +
            f'{selected_name} · Predicted Loaded Draft</div>' +
    ...
    with _t1_summary_ph:
        ...
        st.download_button(
            ...
            mime="image/png", width="stretch", key="dl_t1_png",
        )
...
with tab2:
    section("Enter Target Draft \u2192 Calculated Cargo Volume",
            help_text=(
                "Enter the maximum permissible draft (tide-constrained or bar-crossing draft). "
                "The calculator back-calculates achievable cargo using the "
                "TPC-anchor method (IACS UR S11.4): "
                "displacement at draft = Tropical displacement + TPC x 100 x (draft - TLD), "
                "then cargo = displacement - lightship - deductions. "
                "Dock Water Allowance applied via certified FWA from Q88. "
                "Both hard limits applied: volume <= 98% tank capacity, mass <= Tropical DWT. "
                f"Seawater density: {sw_density:.4f} t/m\u00b3."
            ))
    ...
    with _t2_summary_ph:
        ...
        st.download_button(
            ...
            mime="image/png", width="stretch", key="dl_t2_png",
        )
...
with tab3:
    ...
    st.dataframe(pd.DataFrame(rows_spec), width="stretch", hide_index=True, height=520)
    ...
    for nm, v in sorted(db_now.items()):
        ...
        try:
            ...
            pass
    st.dataframe(pd.DataFrame(comp_rows), width="stretch", hide_index=True, height=480)
...
with tab4:
    ...
    with st.form("add_vessel_form"):
        ...
        if st.form_submit_button("\u2795 Add to Fleet", width="stretch"):
            nn = new_name.strip()
            if not nn:
...
with tab5:
    ...
    with edit_col:
        ...
        with st.form(f"edit_form_{sel_vessel}"):
            ...
            if st.form_submit_button("\U0001f4be Save Changes", width="stretch"):
        ...
        if q88:
            ...
            with st.expander("📋 Raw extraction results (for verification)"):
                _raw_rows = [
                    ...
                    ("FWA — Fresh Water Allowance (mm)", f"{q88.get('fwa_mm'):,.1f}" if q88.get('fwa_mm') else "— (not found — will be derived)"),
                    ("TPC — Tonnes Per Centimetre (MT/cm)", f"{q88.get('tpc_mt_cm'):,.3f}" if q88.get('tpc_mt_cm') else "— (not found — will be derived)"),
            ...
            with st.form("q88_review_form"):
                ...
                with rc3:
                    ...
                    # ── FWA and TPC (Q88 field 1.40) — critical for hydrostatics ──────────
                    # These drive the TPC-anchor draft calculation and DWA density correction.
                    # Pre-filled from Q88 field 1.40; if not found, auto-derived on save.
                    _fwa_extracted = q88.get("fwa_mm")
                    _tpc_extracted = q88.get("tpc_mt_cm")
                    r_fwa = st.number_input(
                        "FWA — Fresh Water Allowance (mm)",
                        min_value=0.0, max_value=800.0,
                        value=float(_fwa_extracted) if _fwa_extracted else 0.0,
                        step=1.0, format="%.1f",
                        help=(
                            "From Q88 field 1.40 (certified by classification society). "
                            "Used for the Dock Water Allowance (DWA) density correction: "
                            "DWA = FWA × (1.025 − ρ_actual) / 0.025. "
                            "Lighter water → vessel sits deeper by DWA mm. "
                            "If 0, the app derives FWA from TPC and summer DWT (ILL 1966 / IACS UR L4). "
                            "Source: ILL 1966, IACS UR L4, BV NR467."
                        ),
                    )
                    r_tpc = st.number_input(
                        "TPC — Tonnes Per Centimetre (MT/cm)",
                        min_value=0.0, max_value=250.0,
                        value=float(_tpc_extracted) if _tpc_extracted else 0.0,
                        step=0.01, format="%.3f",
                        help=(
                            "From Q88 field 1.40 (certified by classification society at summer draft). "
                            "Used as the anchor for draft calculation: "
                            "Δ(T) = Δ_tropical + TPC × 100 × (T − TLD). "
                            "Far more accurate than the proportional method (±0.3% vs ±3–8%). "
                            "If 0, the app derives TPC from waterplane area and Cb (Cw approximation). "
                            "Source: IACS UR S11.4, BV NR467, DNV-GL OTG-09."
                        ),
                    )
                    if not _fwa_extracted or not _tpc_extracted:
                        st.caption(
                            "⚠️ FWA/TPC not found in Q88 text. "
                            "Leave as 0 to auto-derive on save, or enter manually from the "
                            "vessel’s stability booklet (Q88 field 1.40)."
                        )
                ...
                # FWA/TPC status for preview
                _fwa_save = r_fwa if r_fwa > 0 else None
                _tpc_save = r_tpc if r_tpc > 0 else None
                # Derive TPC from waterplane area if not provided (IACS Cw approximation)
                if not _tpc_save:
                    _cw_est  = min(0.995, 0.93 + 0.08 * _auto_cb)
                    _tpc_save = round(r_loa * r_beam * _cw_est * 1.025 / 100, 3)
                # Derive FWA from TPC if not provided (IACS UR L4: FWA = summer_DWT / (4 × TPC))
                if not _fwa_save:
                    _summer_dw_est = r_dwt * 0.97
                    _fwa_raw       = _summer_dw_est / (4.0 * _tpc_save)
                    _fwa_save      = round(_fwa_raw * 1.2483)  # empirical correction from Q88 fleet
                _tpc_label = f"{_tpc_save:.3f} MT/cm {'(Q88 ✓)' if r_tpc > 0 else '(derived)'}"
                _fwa_label = f"{_fwa_save:.0f} mm {'(Q88 ✓)' if r_fwa > 0 else '(derived)'}"
                st.markdown(
                    f"<div class='info-box'>Auto-computed — "
                    ...
                    f"Cb: <strong>{_auto_cb:.4f}</strong> &nbsp;|&nbsp; "
                    f"TPC: <strong>{_tpc_label}</strong> &nbsp;|&nbsp; "
                    f"FWA: <strong>{_fwa_label}</strong>"
                    f"</div>",
                    unsafe_allow_html=True,
                )

                save_q88 = st.form_submit_button(
                    "💾 Save to Fleet", width="stretch",
                )
                ...
                if save_q88:
                    ...
                    if not nn:
                        ...
                        save_vessel({
                            ...
                            "tpc_mt_cm":     round(_tpc_save, 3),
                            "fwa_mm":        round(_fwa_save),
...
<strong>Fields extracted from Q88:</strong> Name · LOA (LBP preferred) · Beam · KTM · GRT · Tropical DWT · Tropical Draft (TLD) · Lightship · Constant · 98% Tank Capacity · FWA (field 1.40) · TPC (field 1.40)<br><br>
<strong>Fields you must enter:</strong> Reference API gravity · Breakwater LAT · Default bunker + FW · Notes<br><br>
<strong>Auto-computed on save:</strong> Vessel class (from DWT) · Block coefficient Cb (from displacement / 1.025 / L×B×TLD) · Tank capacity in US Bbls · TPC (derived from Cb if not in Q88) · FWA (derived from TPC if not in Q88)
</div>
""", unsafe_allow_html=True)
